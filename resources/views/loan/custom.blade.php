...
@section('content')
	...
	<table class="table">
		...
		<tr>
			...
			@if($loan->payments->contains('interest_paid',0))
				<td><b><i><a href="{{route('payment.all.interest',$loan->id)}}" onClick="return confirm('Are you sure?')">Pay All Payable Interest: {{sprintf('Rs.%s/-',round($loan->payments->where('interest_paid',0)->sum('interest_amount')))}}</a></i></b></td>
			@else
				<td><b>Paid</b></td>
			@endif
		</tr>
